...
describe('PoolV4-Locked', function () {
  ...
  describe('Staking Reward', function () {
    describe('Locked', function () {
      ...
      it('unknown address calculate reward returns 0', async function () {
        const { poolV4 } = await loadFixture(deployLocked)
        const randomAddress = ethers.Wallet.createRandom().getAddress()
        expect(await poolV4.calcPendingRewards(randomAddress)).to.equal(0)
      })
